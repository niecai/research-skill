...
通用范式 skill。  
适用于代码、数据、UI、构建、网络、自动化、工具链和方案研究等任何“开始打转”的任务。
...
配套的「具体模式」skill（仍是方法论、领域中立）。  
适用于「产物看起来对了但细节不对」「同一份产物在某些运行时上下文里失效」这类问题：分核心资产层 /
sidecar 元数据层 / 运行时上下文层，找缺失 / 错位 / 越层的控制点。

它比 `top-down-debugging` 更贴近可复用的**具体控制点模式**，但同样**不写任何具体项目 / 产品 /
技术栈**的实例、标识符、路径 —— 所有举例都用领域中立占位。
...
把本仓库当作维护源。各 agent 运行时通常有一份安装副本（如 `<用户目录>/.codex/skills/...`、
`<用户目录>/.claude/skills/...` 等，按工具而定）。
